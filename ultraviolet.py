...
def edit_transaction(index):
    """Edit a transaction"""
    transaction = st.session_state.transactions.iloc[index].to_dict()
    
    st.markdown(f'<div class="section-title">✏️ Edit Transaction: {transaction["Transaction_ID"]}</div>', unsafe_allow_html=True)
    
    with st.form(f"edit_transaction_{index}"):
        col_dt, col_t = st.columns(2)
        with col_dt:
            sale_date = st.date_input("Date*", value=transaction['Date'].date() if hasattr(transaction['Date'], 'date') else datetime.strptime(transaction['Date'], '%Y-%m-%d').date(), key=f"edit_date_{index}")
        with col_t:
            sale_time = st.time_input("Time*", value=datetime.strptime(transaction['Time'], '%H:%M:%S').time() if isinstance(transaction['Time'], str) else transaction['Time'], key=f"edit_time_{index}")

        st.subheader("Item Details")
        col1, col2 = st.columns(2)
        with col1:
            brand = st.text_input("Brand*", value=transaction['Brand'], key=f"edit_brand_{index}")
            color = st.text_input("Color*", value=transaction['Color'], key=f"edit_color_{index}")
            quantity = st.number_input("Quantity*", min_value=1, step=1, value=transaction['Quantity'], key=f"edit_qty_{index}")
        with col2:
            model = st.text_input("Model*", value=transaction['Model'], key=f"edit_model_{index}")
            storage = st.text_input("Storage*", value=transaction['Storage'], key=f"edit_storage_{index}")
            warranty = st.text_input("Warranty Details (Optional)", value=transaction.get('Warranty', ''), key=f"edit_warranty_{index}")
        
        # IMEI field
        imei = st.text_input("IMEI Number (Optional)", value=transaction.get('IMEI', ''), key=f"edit_imei_{index}")
        
        st.subheader("Financial Details")
        col3, col4 = st.columns(2)
        with col3:
            cost_price = st.number_input("Cost Price (PKR)*", min_value=0, value=transaction['Cost_Price'], key=f"edit_cost_{index}")
            paid_amount = st.number_input("Paid Amount (PKR)*", min_value=0, value=transaction['Paid_Amount'], key=f"edit_paid_{index}")
        with col4:
            selling_price = st.number_input("Selling Price (PKR)*", min_value=0, value=transaction['Selling_Price'], key=f"edit_sell_{index}")
            
        st.subheader("Customer Details")
        customer_name = st.text_input("Customer Name*", value=transaction['Customer_Name'], key=f"edit_customer_{index}")
        phone = st.text_input("Phone Number*", value=transaction['Phone'], key=f"edit_phone_{index}")
        cnic = st.text_input("CNIC (Optional)", value=transaction.get('CNIC', ''), key=f"edit_cnic_{index}")
        address = st.text_area("Address (Optional)", value=transaction.get('Address', ''), key=f"edit_address_{index}")
        ...
        if submitted:
            ...
            if not all([brand, model, color, storage, customer_name, phone, selling_price > 0]):
                ...
                st.session_state.transactions.at[index, 'Date'] = sale_date
                st.session_state.transactions.at[index, 'Time'] = sale_time.strftime('%H:%M:%S')
                st.session_state.transactions.at[index, 'Brand'] = brand
                st.session_state.transactions.at[index, 'Model'] = model
                st.session_state.transactions.at[index, 'Color'] = color
                st.session_state.transactions.at[index, 'Storage'] = storage
                st.session_state.transactions.at[index, 'Quantity'] = quantity
                st.session_state.transactions.at[index, 'Selling_Price'] = selling_price
                st.session_state.transactions.at[index, 'Cost_Price'] = cost_price
                st.session_state.transactions.at[index, 'Profit'] = profit
                st.session_state.transactions.at[index, 'Paid_Amount'] = paid_amount
                st.session_state.transactions.at[index, 'Left_Amount'] = left_amount
                st.session_state.transactions.at[index, 'Customer_Name'] = customer_name
                st.session_state.transactions.at[index, 'Phone'] = phone
                st.session_state.transactions.at[index, 'CNIC'] = formatted_cnic
                st.session_state.transactions.at[index, 'Address'] = address
                st.session_state.transactions.at[index, 'Warranty'] = warranty
                st.session_state.transactions.at[index, 'IMEI'] = imei
                st.session_state.transactions.at[index, 'Status'] = 'Completed' if left_amount == 0 else 'Pending'
...
def delete_transaction(index):
    ...
    transaction_id = st.session_state.transactions.iloc[index]['Transaction_ID']
...
def edit_expenditure(index):
    """Edit an expenditure"""
    expenditure = st.session_state.expenditures.iloc[index].to_dict()
    
    st.markdown(f'<div class="section-title">✏️ Edit Expenditure</div>', unsafe_allow_html=True)
    
    with st.form(f"edit_expenditure_{index}"):
        col_dt, col_t = st.columns(2)
        with col_dt:
            exp_date = st.date_input("Date*", value=expenditure['Date'].date() if hasattr(expenditure['Date'], 'date') else datetime.strptime(expenditure['Date'], '%Y-%m-%d').date(), key=f"edit_exp_date_{index}")
        with col_t:
            exp_time = st.time_input("Time*", value=datetime.strptime(expenditure['Time'], '%H:%M:%S').time() if isinstance(expenditure['Time'], str) else expenditure['Time'], key=f"edit_exp_time_{index}")
            
        exp_category = st.text_input("Category*", value=expenditure['Category'], key=f"edit_exp_cat_{index}")
        exp_amount = st.number_input("Amount (PKR)*", min_value=0.0, value=expenditure['Amount'], key=f"edit_exp_amount_{index}")
        exp_description = st.text_area("Description*", value=expenditure['Description'], key=f"edit_exp_desc_{index}")
        
        submitted = st.form_submit_button("💾 Update Expenditure", use_container_width=True)
        if submitted:
            if not all([exp_category, exp_amount > 0, exp_description]):
                ...
                st.session_state.expenditures.at[index, 'Date'] = exp_date
                st.session_state.expenditures.at[index, 'Time'] = exp_time.strftime('%H:%M:%S')
                st.session_state.expenditures.at[index, 'Category'] = exp_category
                st.session_state.expenditures.at[index, 'Amount'] = exp_amount
                st.session_state.expenditures.at[index, 'Description'] = exp_description
...
def delete_expenditure(index):
    """Delete an expenditure"""
    st.session_state.expenditures = st.session_state.expenditures.drop(index).reset_index(drop=True)
    save_data()
    st.success("✅ Expenditure deleted successfully!")
    st.session_state.deleting_exp = None
    time.sleep(1)
    st.rerun()

def edit_payment(index):
    """Edit a payment"""
    payment = st.session_state.payments.iloc[index].to_dict()
    
    st.markdown(f'<div class="section-title">✏️ Edit Payment</div>', unsafe_allow_html=True)
    
    with st.form(f"edit_payment_{index}"):
        col_dt, col_t = st.columns(2)
        with col_dt:
            pay_date = st.date_input("Date*", value=payment['Date'].date() if hasattr(payment['Date'], 'date') else datetime.strptime(payment['Date'], '%Y-%m-%d').date(), key=f"edit_pay_date_{index}")
        with col_t:
            pay_time = st.time_input("Time*", value=datetime.strptime(payment['Time'], '%H:%M:%S').time() if isinstance(payment['Time'], str) else payment['Time'], key=f"edit_pay_time_{index}")
            
        customer_name = st.text_input("Customer Name*", value=payment['Customer_Name'], key=f"edit_pay_customer_{index}")
        phone = st.text_input("Phone Number*", value=payment['Phone'], key=f"edit_pay_phone_{index}")
        cnic = st.text_input("CNIC (Optional)", value=payment.get('CNIC', ''), key=f"edit_pay_cnic_{index}")
        
        col1, col2 = st.columns(2)
        with col1:
            payment_amount = st.number_input("Amount Paid (PKR)*", min_value=0, value=payment['Amount'], key=f"edit_pay_amount_{index}")
        with col2:
            payment_type = st.selectbox("Payment Type*", ['Cash', 'Bank Transfer', 'Online Payment'], 
                                       index=['Cash', 'Bank Transfer', 'Online Payment'].index(payment['Payment_Type']) if payment['Payment_Type'] in ['Cash', 'Bank Transfer', 'Online Payment'] else 0, 
                                       key=f"edit_pay_type_{index}")

        is_advance = st.checkbox("This is a customer advance (not tied to a specific transaction)", value=payment.get('Is_Advance', False), key=f"edit_pay_advance_{index}")
        
        notes = st.text_area("Notes (Optional)", value=payment.get('Notes', ''), key=f"edit_pay_notes_{index}")
        ...
        if submitted:
            if not all([customer_name, phone, payment_amount > 0]):
                ...
                # Update the payment
                st.session_state.payments.at[index, 'Date'] = pay_date
                st.session_state.payments.at[index, 'Time'] = pay_time.strftime('%H:%M:%S')
                st.session_state.payments.at[index, 'Customer_Name'] = customer_name
                st.session_state.payments.at[index, 'Phone'] = phone
                st.session_state.payments.at[index, 'CNIC'] = formatted_cnic
                st.session_state.payments.at[index, 'Amount'] = payment_amount
                st.session_state.payments.at[index, 'Payment_Type'] = payment_type
                st.session_state.payments.at[index, 'Notes'] = notes
                st.session_state.payments.at[index, 'Is_Advance'] = is_advance
                
                save_data()
                st.success("✅ Payment updated successfully!")
                st.session_state.editing_pay = None
                time.sleep(1)
                st.rerun()

def delete_payment(index):
    """Delete a payment"""
    st.session_state.payments = st.session_state.payments.drop(index).reset_index(drop=True)
    save_data()
    ...
    with col1:
        ...
        try:
            ...
            st.download_button(
                ...
                key=f"pdf_{report_period}_{datetime.now().strftime('%Y%m%d%H%M%S%f')}"
            )
        except Exception as e:
...
def data_view_page():
    ...
    with tab1:
        ...
        if st.session_state.get('editing') is not None:
            edit_transaction(st.session_state.editing)
            
        if st.session_state.get('deleting') is not None:
            st.warning("Are you sure you want to delete this transaction?")
            col1, col2 = st.columns(2)
            with col1:
                if st.button("✅ Yes, Delete", use_container_width=True):
                    delete_transaction(st.session_state.deleting)
            with col2:
            ...
            with st.expander(f"Expenditure - {expenditure['Date'].strftime('%Y-%m-%d') if hasattr(expenditure['Date'], 'strftime') else expenditure['Date']}"):
                ...
                with col3:
                    ...
                    if st.button("🗑️ Delete", key=f"delete_exp_{idx}", use_container_width=True):
                        st.session_state.deleting_exp = idx
        
        # Handle edit/delete actions
        if st.session_state.get('editing_exp') is not None:
            edit_expenditure(st.session_state.editing_exp)
            
        if st.session_state.get('deleting_exp') is not None:
            st.warning("Are you sure you want to delete this expenditure?")
            col1, col2 = st.columns(2)
            with col1:
                if st.button("✅ Yes, Delete", use_container_width=True):
                    delete_expenditure(st.session_state.deleting_exp)
            with col2:
                if st.button("❌ Cancel", use_container_width=True):
                    st.session_state.deleting_exp = None
                    st.rerun()
    ...
    with tab3:
        ...
        for idx, payment in filtered_payments.iterrows():
            with st.expander(f"Payment - {payment['Date'].strftime('%Y-%m-%d') if hasattr(payment['Date'], 'strftime') else payment['Date']}"):
                ...
                with col3:
                    if st.button("✏️ Edit", key=f"edit_pay_{idx}", use_container_width=True):
                        st.session_state.editing_pay = idx
                    if st.button("🗑️ Delete", key=f"delete_pay_{idx}", use_container_width=True):
                        st.session_state.deleting_pay = idx
        
        # Handle edit/delete actions
        if st.session_state.get('editing_pay') is not None:
            edit_payment(st.session_state.editing_pay)
            
        if st.session_state.get('deleting_pay') is not None:
            st.warning("Are you sure you want to delete this payment?")
            col1, col2 = st.columns(2)
            with col1:
                if st.button("✅ Yes, Delete", use_container_width=True):
                    delete_payment(st.session_state.deleting_pay)
            with col2:
                if st.button("❌ Cancel", use_container_width=True):
                    st.session_state.deleting_pay = None
                    st.rerun()
...
def main():
    ...
    if 'editing_exp' not in st.session_state:
        st.session_state.editing_exp = None
    if 'deleting_exp' not in st.session_state:
        st.session_state.deleting_exp = None
    if 'editing_pay' not in st.session_state:
        st.session_state.editing_pay = None
